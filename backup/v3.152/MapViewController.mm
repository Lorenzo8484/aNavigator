...
- (void)smoothTick {
    ...
    if (_isSimulating) {
        ...
        // Pre-computa target camera (offset attivo — Waze-style)
    ...
    if (_isInterpolating) {
        ...
        // UNICO evalJS: freccia + camera a 60fps (stessa logica simulazione)
        double camLat = lat;
        double camLon = lon;
        double zoom = 17.0;
        double pitch = 50.0;
        NSString *js = [NSString stringWithFormat:
            ...
            lat, lon, camLat, camLon, zoom, course, pitch];
        [self.webView evaluateJavaScript:js completionHandler:nil];
