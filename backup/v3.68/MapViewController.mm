...
- (void)simulationButtonTapped {
    ...
    // Calcola route PRIMA — OSRM riempie navCoords. POI navigazione + simulazione.
    double fromLat = _lastLocation ? _lastLocation.coordinate.latitude : 44.49;
    double fromLon = _lastLocation ? _lastLocation.coordinate.longitude : 11.34;
    ...
    [self.webView evaluateJavaScript:calcJs completionHandler:^(id result, NSError *error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [self appLog:@"⚠️ Sim: route calcolata (o fallita: %@) — avvio navigazione", error.localizedDescription ?: @"OK"];
