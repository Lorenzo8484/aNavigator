...
- (void)fireBusSimulation {
    ...
    [self pushCameraSettingsToJS];
    [self.webView evaluateJavaScript:@"if(!_navEngineRunning)startNavEngine();" completionHandler:nil];
    // Avvia navTimer per aggiornare nome via/ETA ogni 2 secondi (come auto sim)
    ...
    [self appLog:@"   navTimer partito (bus sim)"];
...
- (void)busSimTick {
    ...
    // Ogni ~1 secondo chiama JS navTick() per aggiornare nome via via navUpdate
    // Stesso percorso dell'auto sim: navTick → getCurrentStreetName → navUpdate → roadStreetLabel
    static int _busNavCounter = 0;
    _busNavCounter++;
    if (_busNavCounter >= 60) {
        _busNavCounter = 0;
        [self.webView evaluateJavaScript:@"navTick();" completionHandler:nil];
    }
