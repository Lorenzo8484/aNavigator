...
- (void)applyCameraSettings {
    ...
    // JS calls for camera - convert meters to zoom (18=~50m, 14=~500m, 12=~2000m)
    double zoom = MAX(10.0, MIN(19.0, 18.0 - log2(MAX(effectiveAltitude, 10.0) / 100.0)));
...
- (void)simulationButtonTapped {
    ...
    // Prova PRIMA a usare navCoords GIÀ ESISTENTI (dalla ricerca)
    [self.webView evaluateJavaScript:@"getRouteCoords()" completionHandler:^(id result, NSError *error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            NSString *jsonStr = (NSString *)result;
            BOOL hasCoords = (jsonStr && [jsonStr length] > 5 && ![jsonStr isEqualToString:@"[]"]);
            
            if (hasCoords) {
                // Route già presente — usa subito! Nessun OSRM, nessun routeReady
                [self appLog:@"✅ Sim: navCoords già pronto (%lu chars) — avvio subito", (unsigned long)[jsonStr length]];
                [self startNavigationTo:dest];
                [self attemptSimCoordsWithRetry:0];
            } else {
                // navCoords vuoto — calcola route OSRM prima
                double fromLat = _lastLocation ? _lastLocation.coordinate.latitude : 44.49;
                double fromLon = _lastLocation ? _lastLocation.coordinate.longitude : 11.34;
                ...
                [self appLog:@"🔄 Sim: navCoords vuoto, calcolo route OSRM..."];
                [self.webView evaluateJavaScript:calcJs completionHandler:^(id result2, NSError *error2) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [self appLog:@"⚠️ Sim: route calcolata (o fallita: %@) — avvio navigazione", error2.localizedDescription ?: @"OK"];
                        [self startNavigationTo:dest];
                        [self attemptSimCoordsWithRetry:0];
                    });
                }];
...
- (void)attemptSimCoordsWithRetry:(int)attempt {
    if (attempt >= 10) {
        [self appLog:@"❌ Sim: timeout 10 tentativi"];
        return;
    }
    NSString *js = @"JSON.stringify(navCoords.map(function(c){return{lat:c[1],lon:c[0]};}))";
    [self.webView evaluateJavaScript:js completionHandler:^(id result, NSError *error) {
        if (error) {
            [self appLog:@"⚠️ Sim coord tentativo %d/10: %@", attempt + 1, error.localizedDescription];
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [self attemptSimCoordsWithRetry:attempt + 1];
            });
            return;
        }
        NSString *jsonStr = (NSString *)result;
        if (!jsonStr || [jsonStr length] < 5 || [jsonStr isEqualToString:@"[]"]) {
            [self appLog:@"⚠️ Sim coord tentativo %d/10: navCoords vuoto", attempt + 1];
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [self attemptSimCoordsWithRetry:attempt + 1];
            });
            return;
        ...
        if (![coords isKindOfClass:[NSArray class]] || coords.count < 2) {
            [self appLog:@"⚠️ Sim coord tentativo %d/10: %lu coord insufficienti", attempt + 1, (unsigned long)[coords count]];
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [self attemptSimCoordsWithRetry:attempt + 1];
            });
            return;
        }
        [self appLog:@"✅ Sim: %lu coordinate — avvio simulazione", (unsigned long)[coords count]];
        _simCoords = coords;
        _simIndex = 0;
        _simStepPerTick = 8.33 * 0.2; // 30 km/h ≈ 1.66m/tick
        NSDictionary *first = coords[0];
        [self setSimulatedLocation:[first[@"lat"] doubleValue] lon:[first[@"lon"] doubleValue] course:0];
        _isSimulating = YES;
        _simTimer = [NSTimer scheduledTimerWithTimeInterval:0.2 target:self selector:@selector(simTick) userInfo:nil repeats:YES];
        [self appLog:@"✅ Simulazione avviata: 30 km/h, 1.66m/tick"];
    }];
}
...
- (void)simTick {
    ...
    double remain = _simStepPerTick; // metri da percorrere in questo tick
    double lat = 0, lon = 0;
    
    while (remain > 0 && _simIndex < (NSInteger)[_simCoords count] - 1) {
        NSDictionary *p1 = _simCoords[_simIndex];
        NSDictionary *p2 = _simCoords[_simIndex + 1];
        double lat1 = [p1[@"lat"] doubleValue], lon1 = [p1[@"lon"] doubleValue];
        double lat2 = [p2[@"lat"] doubleValue], lon2 = [p2[@"lon"] doubleValue];
        
        // Distanza tra i due punti (Haversine)
        double dlat = (lat2 - lat1) * M_PI / 180.0;
        double dlon = (lon2 - lon1) * M_PI / 180.0;
        double a = sin(dlat/2) * sin(dlat/2) + cos(lat1 * M_PI / 180.0) * cos(lat2 * M_PI / 180.0) * sin(dlon/2) * sin(dlon/2);
        double c = 2 * atan2(sqrt(a), sqrt(1-a));
        double segmentDist = 6371000 * c; // metri
        
        if (segmentDist <= remain + 0.001) {
            // Passa al prossimo segmento intero
            remain -= segmentDist;
            _simIndex++;
            // Imposta posizione sul punto successivo
            lat = lat2;
            lon = lon2;
        } else {
            // Interpola tra i due punti
            double frac = remain / segmentDist;
    ...
    // Se arrivato alla fine, usa l'ultimo punto
    if (_simIndex >= (NSInteger)[_simCoords count] - 1 && _simCoords.count > 0) {
        NSDictionary *last = _simCoords.lastObject;
        lat = [last[@"lat"] doubleValue];
        lon = [last[@"lon"] doubleValue];
    }
    
    // Fallback sicurezza
    if (lat == 0 && lon == 0 && _simCoords.count > 0) {
        NSDictionary *first = _simCoords.firstObject;
        lat = [first[@"lat"] doubleValue];
        lon = [first[@"lon"] doubleValue];
    }
    ...
    if (_simIndex < (NSInteger)[_simCoords count] - 1) {
        NSDictionary *p1 = _simCoords[_simIndex];
        NSDictionary *p2 = _simCoords[_simIndex + 1];
        double lat1 = [p1[@"lat"] doubleValue] * M_PI / 180.0;
        double lon1 = [p1[@"lon"] doubleValue] * M_PI / 180.0;
        double lat2 = [p2[@"lat"] doubleValue] * M_PI / 180.0;
        double lon2 = [p2[@"lon"] doubleValue] * M_PI / 180.0;
    ...
    [self setSimulatedLocation:lat lon:lon course:course];
...
- (void)startNavigationTo:(NSDictionary *)destinationDict {
    ...
    if (_lastLocation) {
        ...
        NSString *flyJs = [NSString stringWithFormat:
            ...
            _lastLocation.course >= 0 ? _lastLocation.course + self.cameraHeadingOffset : 0];
