...
@interface MapViewController () <CLLocationManagerDelegate, WKNavigationDelegate, WKScriptMessageHandler, UISearchBarDelegate, UITableViewDataSource, UITableViewDelegate, UIPickerViewDelegate, UIPickerViewDataSource>
@end
...
- (void)setupSearchOverlay {
    ...
    // ⏰ Time picker per bus mode (posizionato in openSearch, ma crealo qui)
    _searchHour = 0;
    _searchMin = 0;
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    if (_searchHour == 0 && _searchMin == 0) {
        ...
        _timePickerLabel.text = [NSString stringWithFormat:@"%02d:%02d (+30min)", _searchHour, _searchMin];
        _timePickerLabel.textColor = [UIColor colorWithRed:0.0 green:0.48 blue:1.0 alpha:1.0];
    }
...
- (void)triggerBusSearch:(NSString *)query {
    ...
    NSString *js;
    if (_searchHour == 0 && _searchMin == 0) {
        // Nessun orario → ricerca vecchia (raggruppa per linea)
        js = [NSString stringWithFormat:@"nativeSearchBusLine('%@')", escaped];
    } else {
        // Con orario → nuova ricerca per singoli trip
        js = [NSString stringWithFormat:@"nativeSearchBusLineByTime('%@', %d, %d)",
              escaped, _searchHour, _searchMin];
    }
...
- (void)openSearch {
    ...
    // Search bar in alto a sinistra, time picker a destra
    CGFloat safeTop = self.view.safeAreaInsets.top;
    self.searchBar.frame = CGRectMake(12, safeTop + 8, w - 155, 44);
    self.searchBar.showsCancelButton = YES;
    
    // ⏰ Time picker a destra della barra di ricerca
    ...
    if (isBus) {
        ...
        _timePicker.frame = CGRectMake(w - 140, safeTop - 6, 128, 58);
        _timePicker.transform = CGAffineTransformMakeScale(0.38, 0.38);
        ...
        _timePickerLabel.hidden = NO;
        _timePickerLabel.frame = CGRectMake(w - 140, safeTop + 48, 128, 16);
        if (_searchHour == 0 && _searchMin == 0) {
            _timePickerLabel.text = @"Tutti gli orari";
            _timePickerLabel.textColor = [[UIColor whiteColor] colorWithAlphaComponent:0.6];
        } else {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (departure && [departure length] > 0) {
        // --- BUS RESULT: orario grande a sx ---
        // Orario a sinistra
        UILabel *timeLabel = [[UILabel alloc] initWithFrame:CGRectMake(12, 4, 70, 36)];
        ...
        UIColor *dirColor = [UIColor colorWithRed:0.0 green:0.48 blue:1.0 alpha:1.0]; // blue default
        if ([dirLabel isEqualToString:@"Ritorno"]) {
            dirColor = [UIColor colorWithRed:1.0 green:0.27 blue:0.27 alpha:1.0]; // rosso
        }
        UILabel *dirLB = [[UILabel alloc] initWithFrame:CGRectMake(12, 38, 70, 16)];
        dirLB.text = dirLabel;
        ...
        // Info a destra (linea + headsign)
        NSString *lineName = r[@"lineNumber"] ?: r[@"name"] ?: @"";
        NSString *headsign = r[@"displaySubtitle"] ?: @"";
        // Rimuovi dirLabel dal subtitle se presente
        NSString *dirPrefix = dirLabel;
        if (dirPrefix.length > 0 && [headsign hasPrefix:dirPrefix]) {
            NSRange rng = [headsign rangeOfString:@" — "];
            if (rng.location != NSNotFound) {
                headsign = [headsign substringFromIndex:rng.location + 3];
            }
        }
        
        UILabel *lineLabel = [[UILabel alloc] initWithFrame:CGRectMake(90, 6, cell.contentView.bounds.size.width - 100, 20)];
        ...
        UILabel *infoLabel = [[UILabel alloc] initWithFrame:CGRectMake(90, 26, cell.contentView.bounds.size.width - 100, 20)];
        infoLabel.text = headsign;
