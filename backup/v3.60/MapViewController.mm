...
- (void)simulationButtonTapped {
    ...
    [self.webView evaluateJavaScript:@"getRouteCoords()" completionHandler:^(id result, NSError *error) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            strongSelf->_simStepPerTick = 4.17 * 0.2; // 15 km/h ≈ 4.17 m/s * 0.2s = 0.83m per tick
            
            // Imposta posizione iniziale sul primo punto del percorso
            ...
            [strongSelf appLog:@"✅ Simulazione avviata: 15 km/h, 0.83m/tick"];
        });
    }];
}
