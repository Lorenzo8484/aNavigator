...
- (void)busSimTick {
    ...
    // Ogni ~1 secondo legge nome via dal percorso OSRM — SOLO lettura, non tocca navCoords
    static int _busStreetCounter = 0;
    _busStreetCounter++;
    if (_busStreetCounter >= 60) {
        _busStreetCounter = 0;
        double sLat = _busSimLat, sLon = _busSimLon;
        [self.webView evaluateJavaScript:[NSString stringWithFormat:@"getStreetNameFromRoute(%f,%f)", sLat, sLon] completionHandler:^(id res, NSError *err) {
