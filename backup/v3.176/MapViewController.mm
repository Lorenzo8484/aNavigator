...
- (void)setupButtons {
    ...
    // 🛣️ Nome via (pill glass in basso, visibile solo in navigazione, testo nero + contorno bianco)
    ...
    if ([message.name isEqualToString:@"speak"]) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (streetName && streetName.length > 0) {
                ...
                self.roadNameLabel.hidden = NO;
                // Testo nero con contorno bianco sottile
                NSDictionary *attrs = @{
...
- (void)enterLayoutEditMode {
    ...
    if (self.roadNameLabel.hidden && !self.isNavigating) {
        ...
        NSDictionary *placeAttrs = @{
            NSForegroundColorAttributeName: [UIColor blackColor],
            NSStrokeColorAttributeName: [UIColor whiteColor],
            NSStrokeWidthAttributeName: @(-3.0),
            NSFontAttributeName: [UIFont boldSystemFontOfSize:20]
        };
        self.roadNameLabel.attributedText = [[NSAttributedString alloc] initWithString:@"Via Nome Strada" attributes:placeAttrs];
...
- (void)exitLayoutEditMode:(BOOL)save {
    ...
    if (self.roadNameLabel.tag == 9999) {
        self.roadNameLabel.hidden = YES;
        self.roadNameLabel.attributedText = [[NSAttributedString alloc] initWithString:@""];
