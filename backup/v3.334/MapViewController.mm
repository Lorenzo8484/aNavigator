...
#define INFINITE_CYCLES 500000

- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component {
    if (pickerView == _dayPicker) return 31 * INFINITE_CYCLES;  // 1-31 × 500k
    if (pickerView == _monthPicker) return 12 * INFINITE_CYCLES; // Gen-Dic × 500k
    if (pickerView == _hourPicker) return 24 * INFINITE_CYCLES;  // ore 00-23 × 500k
    if (pickerView == _minPicker) return 60 * INFINITE_CYCLES;   // minuti 00-59 × 500k
    return 0;
}
...
- (void)searchFieldChanged {
    ...
    if (text.length >= 1) {
        _searchSeq++;
        // Nascondi SUBITO recents prima di lanciare la ricerca async
...
- (void)openSearch {
    ...
    if (isBus) {
        ...
        [_dayPicker selectRow:((_selectedDay - 1) + (INFINITE_CYCLES/2) * 31) inComponent:0 animated:NO];
        [_monthPicker selectRow:((_selectedMonth - 1) + (INFINITE_CYCLES/2) * 12) inComponent:0 animated:NO];
        [_hourPicker selectRow:(_searchHour + (INFINITE_CYCLES/2) * 24) inComponent:0 animated:NO];
        [_minPicker selectRow:(_searchMin + (INFINITE_CYCLES/2) * 60) inComponent:0 animated:NO];
    ...
    [UIView animateWithDuration:0.25 animations:^{
        ...
        // Mostra destinazioni recenti se campo vuoto
        CGFloat safeTop = self.view.safeAreaInsets.top;
        CGFloat tableY;
        if (_isBusSearchMode) {
            tableY = CGRectGetMaxY(_searchContainer.frame) + 8;
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText {
    if (searchText.length >= 1) {
        _searchSeq++;
        // Nascondi SUBITO recents prima della ricerca async
        self.searchResults = [NSMutableArray array];
        [self.searchResultsTable reloadData];
        self.searchResultsTable.hidden = YES;
        if ([SettingsStore shared].transportMode == TransportModeBus) {
            ...
            // Car mode: call nativeSearchOSM via evaluateJavaScript
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message {
    if ([message.name isEqualToString:@"speak"]) {
        ...
        // New format (bus by time): {seq: N, results: [...]}
        ...
        // Stale check: se il messaggio ha un seq, confrontalo con _searchSeq
        // (catturato quando triggerBusSearch e stato chiamato, non quando arriva il callback)
        if (msgSeq >= 0 && msgSeq != _searchSeq) {
            [self appLog:@"⏭ Ignorata callback stale (seq %d != %d)", msgSeq, _searchSeq];
            return;
        }
