...
- (void)showGoButton {
    ...
    sim.frame = CGRectMake(w/2 - 50, h - 85, 100, 38);
    ...
    sim.backgroundColor = [UIColor colorWithRed:0.2 green:0.55 blue:0.2 alpha:0.9];
    sim.layer.cornerRadius = 19;
    sim.tintColor = [UIColor whiteColor];
    sim.titleLabel.font = [UIFont boldSystemFontOfSize:15];
...
- (void)simulationButtonTapped {
    ...
    [self.webView evaluateJavaScript:@"getRouteCoords()" completionHandler:^(id result, NSError *error) {
        ...
        if (![coords isKindOfClass:[NSArray class]] || coords.count < 2) {
            [strongSelf appLog:@"❌ Simulazione: coordinate insufficienti (%lu)", (unsigned long)[coords count]];
            return;
        }
        
        dispatch_async(dispatch_get_main_queue(), ^{
            [strongSelf appLog:@"✅ Simulazione: %lu coordinate caricate", (unsigned long)[coords count]];
            
            // Avvia navigazione reale (route già presente)
            [strongSelf startNavigationTo:strongSelf->_pendingDestDict];
            strongSelf->_pendingDestDict = nil;
            
            // Prepara simulazione sulla route già caricata
            strongSelf->_isSimulating = NO;
            strongSelf->_simCoords = coords;
            strongSelf->_simIndex = 0;
            strongSelf->_simStepPerTick = 8.33 * 0.2; // 30 km/h ≈ 1.66m per tick
            
            // Imposta posizione iniziale sul primo punto
            NSDictionary *first = coords[0];
            double lat = [first[@"lat"] doubleValue];
            double lon = [first[@"lon"] doubleValue];
            [strongSelf setSimulatedLocation:lat lon:lon course:0];
            
            // Avvia timer simulazione
            strongSelf->_isSimulating = YES;
            strongSelf->_simTimer = [NSTimer scheduledTimerWithTimeInterval:0.2 target:strongSelf selector:@selector(simTick) userInfo:nil repeats:YES];
            [strongSelf appLog:@"✅ Simulazione avviata: 30 km/h, 1.66m/tick"];
        });
    }];
}
