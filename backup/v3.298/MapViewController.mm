...
@implementation MapViewController {
    ...
    int _searchHour;
    int _searchMin;
    int _selectedDayIndex; // 0=Lun,1=Mar,2=Mer,3=Gio,4=Ven,5=Sab,6=Dom
    UIPickerView *_timePicker;
    UILabel *_timePickerLabel;
    // Custom barra unica: search + giorni + orario
    UIView *_searchContainer;
    UITextField *_searchField;
    NSMutableArray *_dayButtons; // UIButton × 7
...
- (void)setupSearchOverlay {
    ...
    self.searchBar.searchTextField.textColor = [UIColor colorWithWhite:0.30 alpha:1.0];
    ...
    // TextField
    _searchField = [[UITextField alloc] initWithFrame:CGRectMake(36, 0, 0, 40)];
    ...
    _searchField.delegate = self; // serve UITextFieldDelegate
    ...
    // Day buttons
    NSArray *dayLabels = @[@"L", @"M", @"M", @"G", @"V", @"S", @"D"];
    CGFloat dayBtnX = 0; // Calcolato in openSearch
    for (int i = 0; i < 7; i++) {
        UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
        btn.frame = CGRectMake(0, 0, 26, 26);
        btn.layer.cornerRadius = 13;
        btn.titleLabel.font = [UIFont systemFontOfSize:11 weight:UIFontWeightSemibold];
        [btn setTitle:dayLabels[i] forState:UIControlStateNormal];
        [btn setTitleColor:[UIColor colorWithWhite:0.35 alpha:1.0] forState:UIControlStateNormal];
        [btn setTitleColor:[UIColor whiteColor] forState:UIControlStateSelected];
        btn.backgroundColor = [UIColor clearColor];
        btn.tag = 8000 + i;
        [btn addTarget:self action:@selector(dayButtonTapped:) forControlEvents:UIControlEventTouchUpInside];
        [_searchContainer addSubview:btn];
        [_dayButtons addObject:btn];
    }
    [self updateDaySelection];
    
    // Time display label
    _timeDisplayLabel = [[UILabel alloc] init];
    _timeDisplayLabel.font = [UIFont monospacedDigitSystemFontOfSize:15 weight:UIFontWeightBold];
    _timeDisplayLabel.textColor = [UIColor colorWithWhite:0.35 alpha:1.0];
    _timeDisplayLabel.text = @"Tutti";
    _timeDisplayLabel.textAlignment = NSTextAlignmentCenter;
    _timeDisplayLabel.userInteractionEnabled = YES;
    [_timeDisplayLabel addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(timeLabelTapped)]];
    [_searchContainer addSubview:_timeDisplayLabel];
    
    // ⏰ Time picker (uguale a prima, ma si mostra quando tap su timeDisplayLabel)
    _timePicker = [[UIPickerView alloc] initWithFrame:CGRectMake(0, 0, 128, 58)];
    _timePicker.delegate = self;
    _timePicker.dataSource = self;
    _timePicker.alpha = 0.5;
    _timePicker.transform = CGAffineTransformMakeScale(0.38, 0.38);
    _timePicker.hidden = YES;
    _timePicker.layer.cornerRadius = 12;
    _timePicker.layer.masksToBounds = YES;
    [self.searchOverlay addSubview:_timePicker];
    
    _timePickerLabel = [[UILabel alloc] init];
    _timePickerLabel.text = @"Tutti gli orari";
    _timePickerLabel.textColor = [[UIColor whiteColor] colorWithAlphaComponent:0.6];
    _timePickerLabel.font = [UIFont systemFontOfSize:13];
    _timePickerLabel.textAlignment = NSTextAlignmentCenter;
    _timePickerLabel.hidden = YES;
    [self.searchOverlay addSubview:_timePickerLabel];
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component {
    if (component == 0) return 24; // hours 0-23
...
- (NSAttributedString *)pickerView:(UIPickerView *)pickerView attributedTitleForRow:(NSInteger)row forComponent:(NSInteger)component {
    NSString *text = [NSString stringWithFormat:@"%02ld", (long)row];
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    if (component == 0) _searchHour = (int)row;
    else _searchMin = (int)row;
    
    [self updateTimeDisplay];
    
    // Re-trigger search with current search text
...
- (void)triggerBusSearch:(NSString *)query {
    ...
    NSString *js = [NSString stringWithFormat:@"nativeSearchBusLineByTime('%@', %d, %d, %d)",
          escaped, _searchHour, _searchMin, _selectedDayIndex];
...
#pragma mark - Custom Search Bar (Bus Mode)

- (void)updateDaySelection {
    for (int i = 0; i < 7; i++) {
        UIButton *btn = _dayButtons[i];
...
- (void)dayButtonTapped:(UIButton *)sender {
    int idx = (int)(sender.tag - 8000);
    if (idx >= 0 && idx < 7) {
        _selectedDayIndex = idx;
        [self updateDaySelection];
        [self triggerBusSearchIfNeeded];
    }
}

- (void)timeLabelTapped {
    // Posiziona il picker sotto il time label
    CGRect labelFrame = [_timeDisplayLabel convertRect:_timeDisplayLabel.bounds toView:self.searchOverlay];
    CGFloat pickerX = labelFrame.origin.x + labelFrame.size.width - 128;
    _timePicker.frame = CGRectMake(pickerX, labelFrame.origin.y + labelFrame.size.height + 4, 128, 58);
    _timePicker.transform = CGAffineTransformMakeScale(0.38, 0.38);
    [_timePicker selectRow:_searchHour inComponent:0 animated:NO];
    [_timePicker selectRow:_searchMin inComponent:1 animated:NO];
...
- (void)updateTimeDisplay {
    if (_searchHour == 0 && _searchMin == 0) {
        _timeDisplayLabel.text = @"Tutti";
        _timeDisplayLabel.textColor = [UIColor colorWithWhite:0.35 alpha:1.0];
    } else {
        _timeDisplayLabel.text = [NSString stringWithFormat:@"%02d:%02d", _searchHour, _searchMin];
        _timeDisplayLabel.textColor = [UIColor colorWithRed:0.0 green:0.48 blue:1.0 alpha:1.0];
    }
}
...
- (void)openSearch {
    ...
    if (isBus) {
        // Bus mode: custom container con search + giorni + orario
        self.searchBar.hidden = YES;
        _isBusSearchMode = YES;
        
        CGFloat contW = w - 24;
        _searchContainer.frame = CGRectMake(12, safeTop + 8, contW, 40);
        _searchContainer.hidden = NO;
        
        // Calcola layout: 🔍 | textField | giorni | orario
        // Giorni: 7 bottoni × 22pt + gap = ~178pt
        // Orario: ~55pt
        // Totale destra: ~240pt
        CGFloat rightFixed = 7 * 26 + 6 * 4 + 55 + 14; // ~255
        CGFloat tfWidth = contW - 36 - rightFixed;
        _searchField.frame = CGRectMake(36, 0, MAX(tfWidth, 60), 40);
        
        // Day buttons right of textField
        CGFloat dayStartX = CGRectGetMaxX(_searchField.frame) + 4;
        for (int i = 0; i < 7; i++) {
            UIButton *btn = _dayButtons[i];
            btn.frame = CGRectMake(dayStartX + i * 30, 7, 26, 26);
        }
        
        // Time label at right edge
        CGFloat timeX = dayStartX + 7 * 30 + 4;
        _timeDisplayLabel.frame = CGRectMake(timeX, 0, contW - timeX - 8, 40);
...
- (void)dismissKeyboard {
    if (self.searchActive_Ivar) {
        [self.searchBar resignFirstResponder];
        [_searchField resignFirstResponder];
        // Nascondi time picker se visibile
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText {
    if (searchText.length >= 1) {
        if ([SettingsStore shared].transportMode == TransportModeBus) {
            [self triggerBusSearch:searchText];
        } else {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    // Rimuovi subviews custom precedenti
    for (UIView *sv in cell.contentView.subviews) {
        if (sv.tag >= 9000) [sv removeFromSuperview];
    }
    
    NSString *departure = r[@"departure"];
    
    if (departure && [departure length] > 0) {
        // --- BUS RESULT WITH TIME: orario grande a sx + dettagli completi a dx ---
        // Orario a sinistra
        UILabel *timeLabel = [[UILabel alloc] initWithFrame:CGRectMake(12, 4, 60, 36)];
        timeLabel.text = departure;
        timeLabel.font = [UIFont monospacedDigitSystemFontOfSize:26 weight:UIFontWeightBold];
        ...
        UILabel *dirLB = [[UILabel alloc] initWithFrame:CGRectMake(8, 40, 68, 14)];
        dirLB.text = dirLabel;
        ...
        // Info a destra: displayName (linea + servizio)
        NSString *displayName = r[@"displayName"] ?: @"";
        NSString *subtitle = r[@"displaySubtitle"] ?: @"";
        
        // Rimuovi l'orario dal displayName se presente (es: "16:00  Linea 37 (Feriale)" → "Linea 37 (Feriale)")
        ...
        UILabel *lineLabel = [[UILabel alloc] initWithFrame:CGRectMake(80, 4, cell.contentView.bounds.size.width - 85, 24)];
        ...
        UILabel *infoLabel = [[UILabel alloc] initWithFrame:CGRectMake(80, 26, cell.contentView.bounds.size.width - 85, 32)];
        ...
        // --- NORMAL RESULT (recenti / OSM) ---
        UIListContentConfiguration *cfg = cell.defaultContentConfiguration;
        cfg.text = r[@"displayName"] ?: (r[@"name"] ?: @"");
        cfg.secondaryText = r[@"displaySubtitle"] ?: (r[@"full"] ?: @"");
        cfg.textProperties.color = [UIColor whiteColor];
        cfg.textProperties.font = [UIFont systemFontOfSize:15];
        cfg.secondaryTextProperties.color = [[UIColor whiteColor] colorWithAlphaComponent:0.55];
        cfg.secondaryTextProperties.font = [UIFont systemFontOfSize:11];
        ...
        [self.webView evaluateJavaScript:@"getBusRouteInfo()" completionHandler:^(id infoStr, NSError *e) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [self.webView evaluateJavaScript:@"typeof _busNavData !== 'undefined' && _busNavData.returnStops !== null && _busNavData.returnStops.length >= 2 ? 'yes' : 'no'" completionHandler:^(id r, NSError *e2) {
                    ...
                    [self appLog:@"✅ Mostrati ANDATA (verde)%@", hasRT ? @" + RITORNO (rosso)" : @""];

                    // Bottone SIMULAZIONE BUS (sotto ANDATA/RITORNO)
...
- (void)busSimButtonTapped {
    ...
    // Prende le coordinate OSRM del percorso bus (strade vere) dal JS
    [self.webView evaluateJavaScript:@"JSON.stringify(_busOsrmCoords && _busOsrmCoords.length >= 4 ? _busOsrmCoords.map(function(c){return{lon:c[0],lat:c[1]}}) : [])" completionHandler:^(id res, NSError *err) {
        if (err || ![res isKindOfClass:[NSString class]]) {
            [self appLog:@"❌ Sim Bus: impossibile ottenere percorso OSRM dal JS"];
            return;
        }
        ...
        if (![coords isKindOfClass:[NSArray class]] || coords.count < 4) {
            [self appLog:@"❌ Sim Bus: coordinate OSRM insufficienti (count=%lu), provo con fermate...", (unsigned long)[coords count]];
            // Fallback: usa fermate
            [self.webView evaluateJavaScript:@"JSON.stringify(_busNavData && _busNavData.trip && _busNavData.trip.stops ? _busNavData.trip.stops.map(function(s){return{lat:s.lat,lon:s.lon,name:s.name}}) : [])" completionHandler:^(id res2, NSError *err2) {
                if (err2 || ![res2 isKindOfClass:[NSString class]]) {
                    [self appLog:@"❌ Sim Bus: nessun dato disponibile"];
                    return;
                }
                NSData *jd2 = [(NSString *)res2 dataUsingEncoding:NSUTF8StringEncoding];
                NSArray *stops = [NSJSONSerialization JSONObjectWithData:jd2 options:0 error:nil];
                if (![stops isKindOfClass:[NSArray class]] || stops.count < 2) {
                    [self appLog:@"❌ Sim Bus: fermate insufficienti"];
                    return;
                }
                [self appLog:@"✅ Sim Bus: %lu fermate (fallback)", (unsigned long)[stops count]];
                self->_busSimCoords = [stops copy];
                self->_busSimStepPerTick = 0.1389;
                ...
                if (self.roadNameLabel) self.roadNameLabel.hidden = NO;
                [self fireBusSimulation];
            }];
            return;
        }
        
        [self appLog:@"✅ Sim Bus: %lu coordinate OSRM (strade vere)", (unsigned long)[coords count]];
        self->_busSimCoords = [coords copy];
        self->_busSimStepPerTick = 0.1389; // 0.14 m/tick a 60fps = ~30 km/h (come auto sim)
...
- (void)busSimTick {
    ...
    // Ogni ~200ms (12 tick) legge nome via + aggiorna camera slider
    static int _busStreetCounter = 0;
    _busStreetCounter++;
    if (_busStreetCounter >= 12) {
        _busStreetCounter = 0;
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message {
    if ([message.name isEqualToString:@"speak"]) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            CGFloat rowH = 44;
            if (self.searchResults.count > 0 && [self.searchResults[0] isKindOfClass:[NSDictionary class]]) {
                if (self.searchResults[0][@"departure"]) rowH = 60;
            }
