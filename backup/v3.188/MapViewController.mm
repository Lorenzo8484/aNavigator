...
- (void)setupButtons {
    ...
    // 🛣️ Pill via (120pt: pulito, ETA sotto bordo, nome subito sotto)
    self.roadNameLabel = [[UIView alloc] init];
    self.roadNameLabel.frame = CGRectMake(12, h - 420, w - 24, 120);
    ...
    // Riga inferiore: nome via (bianco, centrato, 2 righe, subito sotto i tempi)
    self.roadStreetLabel = [[UILabel alloc] init];
    self.roadStreetLabel.frame = CGRectMake(12, 34, roadPW - 24, 80);
...
- (void)enterLayoutEditMode {
    ...
    addItem((UIView *)self.roadNameLabel, @"streetname", CGRectMake(12, self.view.bounds.size.height - 420, self.view.bounds.size.width - 24, 120));
...
- (void)loadSavedLayout {
    NSUserDefaults *ud = [NSUserDefaults standardUserDefaults];
    // Reset COMPLETO di tutti i dati salvati per streetname (pulire vecchie versioni)
    // Forza sempre le dimensioni nuove: w = full-width, h = 120
    [ud removeObjectForKey:@"layout_streetname_x"];
    [ud removeObjectForKey:@"layout_streetname_y"];
    ...
    apply(self.roadNameLabel, @"streetname", CGRectMake(12, h - 420, w - 24, 120));
