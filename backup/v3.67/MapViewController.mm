...
- (void)simulationButtonTapped {
    ...
    // Avvia navigazione SUBITO (navBar visibile, camera su posizione)
    [self startNavigationTo:dest];
    
    // Dopo 1.5s ottiene coordinate route da JS (con retry)
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [self startSimulationWithRetry:0];
    });
}

- (void)startSimulationWithRetry:(int)attempt {
    if (attempt >= 5) {
        [self appLog:@"❌ Sim: coordinate non disponibili dopo 5 tentativi"];
        return;
    }
    ...
    [self.webView evaluateJavaScript:js completionHandler:^(id result, NSError *error) {
        if (error || ![result isKindOfClass:[NSString class]]) {
            [self appLog:@"⚠️ Sim: tentativo %d/5 — riprovo...", attempt + 1];
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [self startSimulationWithRetry:attempt + 1];
            });
            return;
        }
        
        NSString *jsonStr = (NSString *)result;
        if ([jsonStr length] < 5 || [jsonStr isEqualToString:@"[]"]) {
            [self appLog:@"⚠️ Sim: navCoords vuoto, tentativo %d/5 — riprovo...", attempt + 1];
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [self startSimulationWithRetry:attempt + 1];
            });
            return;
        ...
        if (!data) { [self appLog:@"❌ Sim: encoding fallito"]; return; }
        ...
        if (![coords isKindOfClass:[NSArray class]] || coords.count < 2) {
            [self appLog:@"❌ Sim: %lu coordinate insufficienti", (unsigned long)[coords count]];
            return;
        }
        
        [self appLog:@"✅ Sim: %lu coordinate caricate", (unsigned long)[coords count]];
        _simCoords = coords;
        _simIndex = 0;
        _simStepPerTick = 8.33 * 0.2; // 30 km/h
        ...
        [self appLog:@"✅ Simulazione avviata: 30 km/h"];
    }];
}
