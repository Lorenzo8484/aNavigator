...
- (void)applyCameraSettings {
    // NON chiamare durante navigazione — JS rAF engine gestisce camera a 60fps
    if (self.isNavigating || _isSimulating) return;
    if (!_lastLocation) {
...
- (void)pushPositionToJSWithLat:(double)lat lon:(double)lon course:(double)course {
    ...
    [self appLog:@"📤 pushPositionToJS(%.6f, %.6f, course=%.0f)", lat, lon, course];
