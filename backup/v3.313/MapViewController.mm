...
- (void)setupSearchOverlay {
    ...
    _selectedDay = 1; // default 1
    _selectedMonth = 6; // default Giugno
    _searchHour = 0;
    _searchMin = 0;
    _isBusSearchMode = NO;
    ...
    // 🎡 Day picker (1-31)
    _dayPicker = [[UIPickerView alloc] initWithFrame:CGRectMake(0, -48, 60, 140)];
    ...
    _monthPicker = [[UIPickerView alloc] initWithFrame:CGRectMake(0, -48, 80, 140)];
    ...
    // ⏰ Time picker (HH:MM) — compatto 100pt con ":"
    _timePicker = [[UIPickerView alloc] initWithFrame:CGRectMake(0, -48, 100, 140)];
    _timePicker.delegate = self;
    _timePicker.dataSource = self;
    _timePicker.backgroundColor = [UIColor clearColor];
    [_searchContainer addSubview:_timePicker];
...
- (NSInteger)numberOfComponentsInPickerView:(UIPickerView *)pickerView {
    if (pickerView == _dayPicker) return 1;
    if (pickerView == _monthPicker) return 1;
    return 3; // timePicker: HH, ":", MM
}

- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component {
    if (pickerView == _dayPicker) return 31; // 1-31
    if (pickerView == _monthPicker) return 12; // Gen-Dic
    if (component == 0) return 24; // hours
    if (component == 1) return 1;  // label ":"
    return 60; // minutes
}
...
- (CGFloat)pickerView:(UIPickerView *)pickerView widthForComponent:(NSInteger)component {
    if (pickerView == _dayPicker) return 60;
    if (pickerView == _monthPicker) return 80;
    if (component == 0) return 38;  // HH
    if (component == 1) return 20;  // ":"
    return 38; // MM
}
...
- (NSAttributedString *)pickerView:(UIPickerView *)pickerView attributedTitleForRow:(NSInteger)row forComponent:(NSInteger)component {
    ...
    if (pickerView == _dayPicker) {
        NSString *text = [NSString stringWithFormat:@"%02ld", (long)(row + 1)];
        return [[NSAttributedString alloc] initWithString:text attributes:@{NSForegroundColorAttributeName: textColor, NSFontAttributeName: font}];
    }
    if (pickerView == _monthPicker) {
        NSArray *months = @[@"Gen",@"Feb",@"Mar",@"Apr",@"Mag",@"Giu",@"Lug",@"Ago",@"Set",@"Ott",@"Nov",@"Dic"];
        return [[NSAttributedString alloc] initWithString:months[row] attributes:@{NSForegroundColorAttributeName: textColor, NSFontAttributeName: font}];
    }
    // Time picker
    if (component == 1) {
        return [[NSAttributedString alloc] initWithString:@":" attributes:@{NSForegroundColorAttributeName: textColor, NSFontAttributeName: [UIFont systemFontOfSize:22 weight:UIFontWeightBold]}];
    }
    return [[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@"%02ld", (long)row] attributes:@{NSForegroundColorAttributeName: textColor, NSFontAttributeName: font}];
}

- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    if (pickerView == _dayPicker) {
        _selectedDay = (int)(row + 1);
    } else if (pickerView == _monthPicker) {
        _selectedMonth = (int)(row + 1);
    } else {
        if (component == 0) _searchHour = (int)row;
        else if (component == 2) _searchMin = (int)row;
    }
...
- (void)searchFieldChanged {
    ...
    if (text.length >= 1) {
        // Nascondi recents/subito quando si digita
...
- (void)triggerBusSearch:(NSString *)query {
    ...
    NSString *js = [NSString stringWithFormat:@"nativeSearchBusLineByTime('%@', %d, %d, %d, %d)",
          escaped, _searchHour, _searchMin, _selectedDay, _selectedMonth];
...
- (void)openSearch {
    ...
    if (isBus) {
        ...
        // Layout: 🔍 (26pt) | textField | dayPicker (60pt) | monthPicker (80pt) | timePicker (100pt)
        CGFloat tfWidth = contW - 40 - 60 - 80 - 100 - 12;
        _searchField.frame = CGRectMake(40, 0, MAX(tfWidth, 50), 44);
        
        // Day picker a destra del textField
        CGFloat pickersStartX = CGRectGetMaxX(_searchField.frame) + 4;
        _dayPicker.frame = CGRectMake(pickersStartX, -48, 60, 140);
        _monthPicker.frame = CGRectMake(pickersStartX + 62, -48, 80, 140);
        _timePicker.frame = CGRectMake(pickersStartX + 144, -48, 100, 140);
        
        [_dayPicker selectRow:(_selectedDay - 1) inComponent:0 animated:NO];
        [_monthPicker selectRow:(_selectedMonth - 1) inComponent:0 animated:NO];
        [_timePicker selectRow:_searchHour inComponent:0 animated:NO];
        [_timePicker selectRow:0 inComponent:1 animated:NO]; // ":"
        [_timePicker selectRow:_searchMin inComponent:2 animated:NO];
    ...
    [UIView animateWithDuration:0.25 animations:^{
        ...
        // Mostra destinazioni recenti (solo in car mode)
        if (!_isBusSearchMode) {
            CGFloat safeTop = self.view.safeAreaInsets.top;
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText {
    if (searchText.length >= 1) {
        ...
        if (_isBusSearchMode) return; // Bus mode: niente recents
        // Mostra destinazioni recenti
        self.searchResults = [[[SettingsStore shared] recentDestinations] mutableCopy];
        [self.searchResultsTable reloadData];
        self.searchResultsTable.hidden = (self.searchResults.count == 0);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (departure && [departure length] > 0) {
        ...
        // Colonna orologio
        UILabel *timeLabel = [[UILabel alloc] initWithFrame:CGRectMake(12, 0, 65, 60)];
        ...
        UILabel *lineLabel = [[UILabel alloc] initWithFrame:CGRectMake(90, 4, cell.contentView.bounds.size.width - 95, 24)];
        ...
        UILabel *infoLabel = [[UILabel alloc] initWithFrame:CGRectMake(90, 26, cell.contentView.bounds.size.width - 95, 32)];
...
- (void)showGoButton {
    ...
    if ([SettingsStore shared].transportMode == TransportModeBus && self->_pendingDestDict) {
        // Leggi la direzione selezionata dall'utente
        ...
        [self.webView evaluateJavaScript:@"getBusRouteInfo()" completionHandler:^(id infoStr, NSError *e) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                BOOL isRitornoSelected = (selectedDir == 1);
                
                // Primo bottone = direzione SELEZIONATA
                ...
                NSString *btn1Label = isRitornoSelected ? @"RITORNO" : @"ANDATA";
                UIColor *btn1Color = isRitornoSelected ? [UIColor colorWithRed:0.80 green:0.15 blue:0.15 alpha:0.95] : [UIColor colorWithRed:0.13 green:0.70 blue:0.25 alpha:0.95];
                btn1.backgroundColor = btn1Color;
                
                UILabel *label1 = [[UILabel alloc] init];
                label1.frame = CGRectMake(4, 6, btnW - 8, 22);
                label1.text = btn1Label;
                ...
                route1.text = isRitornoSelected ? ritornoRoute : andataRoute;
                ...
                // --- Secondo bottone (direzione OPPOSTA) ---
                [self.webView evaluateJavaScript:@"typeof _busNavData !== 'undefined' && _busNavData.returnStops !== null && _busNavData.returnStops.length >= 2 ? 'yes' : 'no'" completionHandler:^(id r, NSError *e2) {
                    ...
                    if (hasRT) {
                        NSString *btn2Label = isRitornoSelected ? @"ANDATA" : @"RITORNO";
                        UIColor *btn2Color = isRitornoSelected ? [UIColor colorWithRed:0.13 green:0.70 blue:0.25 alpha:0.95] : [UIColor colorWithRed:0.80 green:0.15 blue:0.15 alpha:0.95];
                        ...
                        ritRoute.text = isRitornoSelected ? andataRoute : ritornoRoute;
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message {
    if ([message.name isEqualToString:@"speak"]) {
        ...
        // Results from nativeSearchOSM / nativeSearchBusLineByTime
        NSArray *results = nil;
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            // Sostituisce COMPLETAMENTE i risultati (niente recents sotto)
            ...
            if (_isBusSearchMode) {
                // Bus mode: tableY sotto il container custom
