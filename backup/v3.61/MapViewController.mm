...
- (void)simTick {
    if (!_isSimulating || !_simCoords || _simIndex >= [_simCoords count] - 1) {
        if (_isSimulating) {
    ...
    // Trova prossimo punto e calcola avanzamento
    double remain = _simStepPerTick; // metri da percorrere in questo tick
    double lat = 0, lon = 0;
    NSInteger startIdx = _simIndex;
    ...
    while (remain > 0 && _simIndex < (NSInteger)[_simCoords count] - 1) {
        ...
        // Distanza tra i due punti (Haversine approssimato)
    ...
    if (_simIndex < (NSInteger)[_simCoords count] - 1) {
        NSDictionary *p1 = _simCoords[_simIndex];
        NSDictionary *p2 = _simCoords[_simIndex + 1];
        lat = [p1[@"lat"] doubleValue];
        lon = [p1[@"lon"] doubleValue];
    }
    if (lat == 0 && lon == 0 && _simIndex < (NSInteger)[_simCoords count]) {
        NSDictionary *last = _simCoords[_simIndex];
