...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component {
    ...
    if (pickerView == _hourPicker) return 24; // ore 00-23
    if (pickerView == _minPicker) return 60; // minuti 00-59
    return 0;
}
...
- (void)openSearch {
    ...
    if (isBus) {
        ...
        [_hourPicker selectRow:_searchHour inComponent:0 animated:NO];
        [_minPicker selectRow:_searchMin inComponent:0 animated:NO];
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message {
    if ([message.name isEqualToString:@"speak"]) {
        ...
        int seqAtCall = _searchSeq;
        dispatch_async(dispatch_get_main_queue(), ^{
            // Ignora callback stale da ricerche precedenti
            if (seqAtCall != _searchSeq) return;
