...
- (void)applyCameraSettings {
    ...
    // Offset: durante navigazione AVANTI (per far sembrare camera dietro col pitch 60°), altrimenti laterale
    CLLocationCoordinate2D centerCoord = _lastLocation.coordinate;
    double offsetLat, offsetLon;
    if (self.isNavigating) {
        // AVANTI: centro davanti alla freccia + pitch alto = effetto 3rd-person (freccia in basso, strada in alto)
    ...
    // JS calls for camera - zoom based on altitude, max zoom 20.5 per ~20m reali
    double zoom = MAX(10.0, MIN(20.5, 18.0 - log2(MAX(effectiveAltitude, 10.0) / 100.0)));
