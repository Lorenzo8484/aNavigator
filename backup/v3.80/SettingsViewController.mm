...
typedef NS_ENUM(NSInteger, RowType) { RowTypeChevron, RowTypeSwitch, RowTypeSlider };
@interface SettingsRow : NSObject
@property (nonatomic,copy) NSString *title,*value,*key; @property (nonatomic) RowType type; @property (nonatomic) BOOL on; @property (nonatomic) float sliderValue;
...
+ (instancetype)slider:(NSString*)t val:(float)val { SettingsRow *r=[SettingsRow new]; r.title=t; r.key=t; r.type=RowTypeSlider; r.sliderValue=val; return r; }
@end
...
@property (nonatomic,strong) UILabel *rowTitle,*rowValue; @property (nonatomic,strong) UIImageView *rowChevron; @property (nonatomic,strong) UISwitch *rowSwitch; @property (nonatomic,strong) UISlider *rowSlider; @property (nonatomic,strong) UIView *divider;
...
- (void)config:(SettingsRow*)row {
    self.rowKey=row.key;
    _rowTitle.text=LOC(row.title);
    if (row.type==RowTypeSlider) {
        _rowChevron.hidden=YES; _rowValue.hidden=NO; _rowSwitch.hidden=YES;
        _rowValue.text=[NSString stringWithFormat:@"%.0f%%",(row.type==RowTypeSlider)?row.sliderValue*100:0];
        _rowValue.textColor=[[UIColor whiteColor] colorWithAlphaComponent:0.55];
        _rowTitle.textColor=[[UIColor whiteColor] colorWithAlphaComponent:1.0];
        // Add slider to contentView
        if (!self->_rowSlider) {
            self->_rowSlider=[[UISlider alloc] init];
            _rowSlider.minimumValue=0.0; _rowSlider.maximumValue=1.0;
            _rowSlider.tintColor=[UIColor systemBlueColor];
            _rowSlider.translatesAutoresizingMaskIntoConstraints=NO;
            [self.contentView addSubview:_rowSlider];
            [NSLayoutConstraint activateConstraints:@[
                [_rowSlider.trailingAnchor constraintEqualToAnchor:self.contentView.trailingAnchor constant:-22],
                [_rowSlider.centerYAnchor constraintEqualToAnchor:self.contentView.centerYAnchor],
                [_rowSlider.widthAnchor constraintEqualToConstant:120],
            ]];
        }
        _rowSlider.hidden=NO; _rowSlider.value=row.sliderValue;
        [_rowSlider removeTarget:nil action:NULL forControlEvents:UIControlEventValueChanged];
        [_rowSlider addTarget:self action:@selector(rowSliderChanged:) forControlEvents:UIControlEventValueChanged];
    } else {
        BOOL s=(row.type==RowTypeSwitch);
        _rowChevron.hidden=s; _rowValue.hidden=s; _rowSwitch.hidden=!s; _rowSwitch.on=row.on;
        if (_rowSlider) _rowSlider.hidden=YES;
        [_rowSwitch addTarget:self action:@selector(rowSwitchChanged:) forControlEvents:UIControlEventValueChanged];
...
- (void)rowSliderChanged:(UISlider*)sl {
    SettingsStore *st=[SettingsStore shared];
    st.mapBrightness = sl.value;
    [st save];
    // Update label
    UITableViewCell *cell = (UITableViewCell*)self.superview;
    while (cell && ![cell isKindOfClass:[SettingsCell class]]) cell = (UITableViewCell*)cell.superview;
    if (cell) {
        SettingsCell *sc = (SettingsCell*)cell;
        sc.rowValue.text = [NSString stringWithFormat:@"%.0f%%", (double)(sl.value * 100)];
    }
    // Apply to map
...
@property (nonatomic,strong) UISlider *altS,*pitS,*offS,*vertS,*traspS,*busYS,*busXS,*busSS,*busRS,*arrowRS;
@property (nonatomic,strong) UILabel *altV,*pitV,*offV,*vertV,*traspV,*busYV,*busXV,*busSV,*busRV,*arrowRV;
@end
...
- (void)setupSliders {
    ...
    // 4 sliders camera (240) + header Temi (20) + 4 sliders temi (240) + header Bus (20) + 4 sliders bus (240) + header Freccia (20) + 1 slider (60) + margin + brightness = 960
    CGFloat SDH=1150;
    [NSLayoutConstraint activateConstraints:@[
    ...
    float mop=[ud objectForKey:[self udk:@"menu_op"]]?[ud floatForKey:[self udk:@"menu_op"]]:1.0;
    self.mapVC.cameraAltitude=alt; self.mapVC.cameraPitch=pit; self.mapVC.cameraOffset=off; self.mapVC.cameraVerticalOffset=[ud objectForKey:[self udk:@"cam_voff"]]?[ud floatForKey:[self udk:@"cam_voff"]]:0;
...
- (void)rebuildSliders {
    ...
    [self mkSX:0 top:30  t:LOC(@"Altezza")         mn:20  mx:2500 val:altV fn:^(UISlider*sl){ self.mapVC.cameraAltitude=sl.value; self.altV.text=[NSString stringWithFormat:@"%.0f m",sl.value]; } minL:@"20m" midL:@"1500" maxL:@"2500" s:&a1 l:&v1 save:@"cam_alt" cam:YES];
    [self mkSX:1 top:90  t:LOC(@"Angolazione")     mn:0   mx:90   val:pitV fn:^(UISlider*sl){ self.mapVC.cameraPitch=sl.value; self.pitV.text=[NSString stringWithFormat:@"%.0f°",sl.value]; } minL:@"0°" midL:@"45°" maxL:@"90°" s:&a2 l:&v2 save:@"cam_pit" cam:YES];
    ...
    [self mkSX:3 top:210 t:LOC(@"Su/Giù")    mn:-100 mx:100   val:0 fn:^(UISlider*sl){
        self.mapVC.cameraVerticalOffset = sl.value;
        self.vertV.text=[NSString stringWithFormat:@"%.0f m",sl.value];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"MapSettingsChanged" object:nil];
    } minL:@"Su" midL:@"0" maxL:@"Giù" s:&a4 l:&v4 save:@"cam_voff" cam:YES];
    _altS=a1;_pitS=a2;_offS=a3;_vertS=a4; _altV=v1;_pitV=v2;_offV=v3;_vertV=v4;
    ...
    [self mkSX:4 top:336 t:LOC(@"Trasparenza") mn:0.1 mx:1.0 val:mopV fn:^(UISlider*sl){
        ...
        self.mapVC.menuOpacity = sl.value;
    ...
    } minL:LOC(@"Fredda") midL:LOC(@"Neutra") maxL:LOC(@"Calda") s:&a8 l:&v8 save:@"theme_tin" cam:NO];
    ...
    [self mkSX:10 top:716 t:LOC(@"Ingrandimento") mn:0.1 mx:2.5 val:busS fn:^(UISlider*sl){
        store.busScale=sl.value; [store save];
        [self.mapVC.busVC updateBusScale];
        self->_busSV.text=[NSString stringWithFormat:@"%.1fx",sl.value];
    } minL:LOC(@"0.1x") midL:@"" maxL:LOC(@"2.5x") s:&a11 l:&v11 save:@"busS" cam:NO];
    ...
    // ── 🎨 Tema scuro ──
    BOOL ds = [SettingsStore shared].darkTheme;
    UILabel *tsh = [[UILabel alloc] init]; tsh.text = LOC(@"🎨 Tema scuro");
    tsh.font = [UIFont boldSystemFontOfSize:14];
    tsh.textColor = [[UIColor whiteColor] colorWithAlphaComponent:0.65]; tsh.translatesAutoresizingMaskIntoConstraints=NO;
    [self.slidersView addSubview:tsh];
    [NSLayoutConstraint activateConstraints:@[
        [tsh.topAnchor constraintEqualToAnchor:self.slidersView.topAnchor constant:840],
        [tsh.leadingAnchor constraintEqualToAnchor:self.slidersView.leadingAnchor constant:4],
    ]];
    
    UISwitch *tsw = [[UISwitch alloc] init];
    tsw.on = ds;
    tsw.onTintColor = [UIColor colorWithRed:0.18 green:0.49 blue:1.0 alpha:1.0];
    tsw.translatesAutoresizingMaskIntoConstraints = NO;
    ...
    tsv.text = ds ? LOC(@"Attivo") : LOC(@"Disattivo");
    tsv.font = [UIFont systemFontOfSize:12];
    tsv.textColor = [[UIColor whiteColor] colorWithAlphaComponent:0.5];
    tsv.translatesAutoresizingMaskIntoConstraints = NO;
...
- (void)mkSX:(int)tag top:(CGFloat)top t:(NSString*)t mn:(float)mn mx:(float)mx val:(float)val fn:(void(^)(UISlider*))fn minL:(NSString*)minL midL:(NSString*)midL maxL:(NSString*)maxL s:(UISlider**)sp l:(UILabel**)lp save:(NSString*)key cam:(BOOL)isCam {
    ...
    if(isCam){
        ...
        // Applica impostazioni camera immediatamente all'inizializzazione
        [self.mapVC applyCameraSettings];
    }
...
- (void)sliderChanged:(UISlider*)sl {
    ...
    if(vl){
        float v=sl.value;
        if(sl.tag==0) vl.text=[NSString stringWithFormat:@"%.0f m",v];
        else if(sl.tag==1) vl.text=[NSString stringWithFormat:@"%.0f°",v];
        else if(sl.tag==2) vl.text=[NSString stringWithFormat:@"%.1f m",v];
        else if(sl.tag==3) vl.text=[NSString stringWithFormat:@"%.0f°",v];
        else if(sl.tag==4||sl.tag==6) vl.text=[NSString stringWithFormat:@"%.0f%%",v*100];
        else if(sl.tag==5) vl.text=[NSString stringWithFormat:@"%.0f%%",v*100];
        else if(sl.tag==7) vl.text=[NSString stringWithFormat:@"%.1f",v];
        else if(sl.tag==8||sl.tag==9) vl.text=[NSString stringWithFormat:@"%.1f",v];
        else if(sl.tag==10) vl.text=[NSString stringWithFormat:@"%.1fx",v];
        else if(sl.tag==11||sl.tag==12) vl.text=[NSString stringWithFormat:@"%.0f°",v];
        else if(sl.tag==13) vl.text=[NSString stringWithFormat:@"%.0f%%",v*100];
        else vl.text=[NSString stringWithFormat:@"%.0f%%",v*100];
    }
    NSUserDefaults *ud=[NSUserDefaults standardUserDefaults];
    [ud setFloat:sl.value forKey:[self udk:key]];
    [ud synchronize];
    if(sl.tag>=0 && sl.tag<=2) [self.mapVC applyCameraSettings];
...
- (void)busSliderUp:(UISlider*)sl {
    [UIView animateWithDuration:0.25 animations:^{
        ...
        self.mapVC.busVC.view.hidden = YES;
        self.mapVC.busVC.view.alpha = 0;
    }];
}
...
- (void)themeSwitchChanged:(UISwitch*)sw {
    [SettingsStore shared].darkTheme = sw.on;
    [[SettingsStore shared] save];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"MapSettingsChanged" object:nil];
}
