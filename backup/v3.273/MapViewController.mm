...
- (void)busSimButtonTapped {
    ...
    // Prende il nome linea bus per la pill via
    [self.webView evaluateJavaScript:@"_busNavData && _busNavData.line ? _busNavData.line : ''" completionHandler:^(id res, NSError *err) {
        if ([res isKindOfClass:[NSString class]] && [(NSString *)res length] > 0) {
            self.roadStreetLabel.text = [NSString stringWithFormat:@"🚌 Linea %@", res];
        } else {
            self.roadStreetLabel.text = @"🚌 Bus";
        }
    }];
...
- (void)busSimTick {
    ...
    if (_busSimIndex == 0 && curLat == 0 && curLon == 0) {
        ...
        curLat = [first[@"lat"] doubleValue];
        curLon = [first[@"lon"] doubleValue];
        // Mostra nome linea bus nella pill via
        if (self.roadStreetLabel && self.roadStreetLabel.text.length == 0) {
            self.roadStreetLabel.text = @"🚌 Bus";
        }
    }
