...
- (void)setSimulatedLocation:(double)lat lon:(double)lon course:(double)course {
    ...
    // Camera si aggiorna PRIMA della freccia — così _cameraHeading è già corretto per updateArrowRotation
    if (_isSimulating) {
        [self applyCameraSettings];
    }
    
    // Aggiorna JS
    // updatePosition OGNI 6 TICK (~100ms) — elimina tremolio a zoom vicino
    _posUpdateCounter++;
    if (_posUpdateCounter >= 6) {
        _posUpdateCounter = 0;
    ...
    // updateArrowRotation usa _cameraHeading che è stato appena aggiornato da applyCameraSettings
