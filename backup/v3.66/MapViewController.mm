...
- (void)simulationButtonTapped {
    ...
    // Salva destinazione per usarla nel callback simCoords
    self->_pendingSimDestDict = self->_pendingDestDict;
    [[SettingsStore shared] addRecentDestination:self->_pendingDestDict];
    self->_pendingDestDict = nil;
    [self appLog:@"   Richiedo coordinate route..."];
    
    // Chiede a JS di inviare le coordinate via messageHandler (fire-and-forget)
    [self.webView evaluateJavaScript:@"sendSimCoords()" completionHandler:nil];
}

- (void)startSimulationWithCoords:(NSArray *)coords {
    [self appLog:@"✅ Simulazione: %lu coordinate ricevute", (unsigned long)[coords count]];
    
    // Avvia navigazione (route già presente dal tap)
    [self startNavigationTo:self->_pendingSimDestDict];
    self->_pendingSimDestDict = nil;
    
    // Prepara simulazione
    _simCoords = coords;
    _simIndex = 0;
    _simStepPerTick = 8.33 * 0.2; // 30 km/h ≈ 1.66m per tick
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message {
    if ([message.name isEqualToString:@"speak"]) {
        ...
        NSString *jsonStr = message.body;
        if (![jsonStr isKindOfClass:[NSString class]]) { [self appLog:@"❌ simCoords: body not string"]; return; }
        NSData *data = [jsonStr dataUsingEncoding:NSUTF8StringEncoding];
        if (!data) { [self appLog:@"❌ simCoords: encoding fallito"]; return; }
        ...
        if (![coords isKindOfClass:[NSArray class]] || coords.count < 2) {
            [self appLog:@"❌ simCoords: %lu coord insufficienti", (unsigned long)[coords count]];
            return;
        }
        dispatch_async(dispatch_get_main_queue(), ^{
            [self startSimulationWithCoords:coords];
        });
    }
}
