...
- (void)applyCameraSettings {
    ...
    // Offset: durante navigazione DIETRO la freccia, altrimenti laterale
    CLLocationCoordinate2D centerCoord = _lastLocation.coordinate;
    double offsetLat, offsetLon;
    if (self.isNavigating) {
        // DIETRO la freccia: camera offset indietro rispetto alla direzione di marcia
        // headingRad = heading + cameraHeadingOffset (se l'utente ha impostato uno sbilanciamento)
...
- (void)fireSimulation {
    ...
    _simTimer = [NSTimer scheduledTimerWithTimeInterval:0.05 target:self selector:@selector(simTick) userInfo:nil repeats:YES];
    [self appLog:@"✅ Simulazione avviata: %lu coord, %.3f m/tick, 20fps", (unsigned long)[_simCoords count], _simStepPerTick];
}
...
- (void)startSmoothTimer {
    ...
    _smoothTimer = [NSTimer scheduledTimerWithTimeInterval:0.05 target:self selector:@selector(smoothTick) userInfo:nil repeats:YES];
}
...
- (void)smoothTick {
    ...
    if (!_isInterpolating || !self.isNavigating) return;
    ...
    t = t * t * (3.0 - 2.0 * t); // smoothstep per accelerazione/decelerazione
    ...
    // Rotazione freccia verso la direzione di movimento
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message {
    if ([message.name isEqualToString:@"speak"]) {
        ...
        if ([coords isKindOfClass:[NSArray class]] && coords.count >= 2) {
            dispatch_async(dispatch_get_main_queue(), ^{
                [self appLog:@"✅ simCoords: %lu coordinate salvate, stepPerTick=%.3f", (unsigned long)[coords count], 0.4165];
                    _simCoords = coords;
                    _simIndex = 0;
                    _simStepPerTick = 0.4165;
            });
        }
