...
- (void)fireSimulation {
    if (!_simCoords || _simCoords.count < 2) {
        [self appLog:@"❌ fireSimulation: _simCoords insufficienti"];
        return;
    }
    _simIndex = 0;
    _isSimulating = YES;
    ...
    [self appLog:@"✅ Simulazione avviata: %lu coord, 30 km/h, 0.2s/tick", (unsigned long)[_simCoords count]];
}
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message {
    if ([message.name isEqualToString:@"speak"]) {
        ...
        NSString *jsonStr = message.body;
        if (![jsonStr isKindOfClass:[NSString class]]) return;
        [self appLog:@"📩 simCoords ricevute (%lu chars)", (unsigned long)[jsonStr length]];
        NSData *data = [jsonStr dataUsingEncoding:NSUTF8StringEncoding];
        ...
        if ([coords isKindOfClass:[NSArray class]] && coords.count >= 2) {
            dispatch_async(dispatch_get_main_queue(), ^{
                [self appLog:@"✅ simCoords: %lu coordinate salvate", (unsigned long)[coords count]];
                _simCoords = coords;
                _simIndex = 0;
                _simStepPerTick = 1.666; // 30 km/h = 1.66m/tick
            });
        }
