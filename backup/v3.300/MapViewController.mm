...
- (void)setupSearchOverlay {
    ...
    _searchContainer.clipsToBounds = NO;
    _searchContainer.hidden = YES;
    [self.searchOverlay addSubview:_searchContainer];
    
    // Icona lente
    UIImageView *magIcon = [[UIImageView alloc] initWithFrame:CGRectMake(10, 11, 20, 20)];
    ...
    // TextField
    _searchField = [[UITextField alloc] initWithFrame:CGRectMake(36, 0, 0, 44)];
    ...
    // 🎡 Day picker (rotellina su-giu, stile orologio)
    _dayPicker = [[UIPickerView alloc] initWithFrame:CGRectMake(0, 0, 160, 58)];
    _dayPicker.delegate = self;
    _dayPicker.dataSource = self;
    _dayPicker.transform = CGAffineTransformMakeScale(0.38, 0.38);
    _dayPicker.backgroundColor = [UIColor clearColor];
    _dayPicker.layer.cornerRadius = 8;
    _dayPicker.layer.masksToBounds = YES;
    ...
    _timePicker.layer.cornerRadius = 8;
...
- (void)openSearch {
    ...
    if (isBus) {
        ...
        // Layout: 🔍 | textField | dayPicker | timePicker
        CGFloat pickersW = 160 + 128; // dayPicker + timePicker (frame pre-scale)
        CGFloat scaledPickersW = (160 + 128) * 0.38; // ~109pt visivi
        CGFloat tfWidth = contW - 36 - scaledPickersW - 8;
        _searchField.frame = CGRectMake(36, 0, MAX(tfWidth, 60), 44);
        
        // Day picker a destra del textField
        CGFloat pickersStartX = CGRectGetMaxX(_searchField.frame) + 4;
        _dayPicker.frame = CGRectMake(pickersStartX / 0.38, 0, 160, 58);
        _timePicker.frame = CGRectMake((pickersStartX + 160 * 0.38 + 2) / 0.38, 0, 128, 58);
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (r[@"departure"]) return 52; // bus result
    ...
    if ([message.name isEqualToString:@"speak"]) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (self.searchResults.count > 0 && [self.searchResults[0] isKindOfClass:[NSDictionary class]]) {
                if (self.searchResults[0][@"departure"]) rowH = 52;
            }
