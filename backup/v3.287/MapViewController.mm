...
- (void)viewDidLayoutSubviews {
    ...
    // Road name label (in basso, larghezza piena)
    self.roadNameLabel.frame = CGRectMake(12, h - 420, w - 24, 40);
    self.roadStreetLabel.frame = CGRectMake(8, 0, w - 40, 40);
    
    // Nav bar (in alto, larghezza piena)
    self.navBar.frame = CGRectMake(0, 0, w, 180);
    
    // Go / Sim buttons (centrati in basso, solo se esistenti)
    if (self->_goButton) {
        self->_goButton.frame = CGRectMake(w/2 - 70, h - 150, 140, 50);
    }
    if (self->_simButton) {
        self->_simButton.frame = CGRectMake(w/2 - 70, h - 90, 140, 42);
    }
    if (self->_busSimButton) {
        self->_busSimButton.frame = CGRectMake(w/2 - 70, h - 90, 140, 36);
    }
}
...
- (void)applyCameraSettings {
    // Durante navigazione/simulazione: pusha valori al JS rAF engine
    if (self.isNavigating || _isSimulating) {
        if (!_lastLocation) return;
        [self pushCameraSettingsToJS];
        // NON tornare — continua per forzare camera attaccata con animateCamera
    }
    if (!_lastLocation) {
...
- (void)setSimulatedLocation:(double)lat lon:(double)lon course:(double)course {
    ...
    if (_camCounter >= 3) {
        _camCounter = 0;
        [self pushPositionToJS];
        // Forza camera attaccata ogni ~50ms anche per auto sim
        [self applyCameraSettings];
    }
}
...
- (void)busSimTick {
    ...
    if (_busStreetCounter >= 12) {
        ...
        // Aggiorna camera slider + FORZA camera attaccata (jumpTo, non rAF chase)
        [self pushCameraSettingsToJS];
        [self busApplyCameraSettings];
    }
