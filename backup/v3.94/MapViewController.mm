...
- (void)applyAllSettings {
    ...
    if (st.nightMode) {
        ...
        [self.webView evaluateJavaScript:@"setMapType(1)" completionHandler:^(id r, NSError *err) {
            ...
            else [self appLog:@"✓ setMapType(1) → %@", r];
        }];
    } else {
        ...
        [self.webView evaluateJavaScript:[NSString stringWithFormat:@"setMapType(%ld)", mt] completionHandler:^(id r, NSError *err) {
            ...
            else [self appLog:@"✓ setMapType(%ld) → %@", mt, r];
        }];
    }
...
- (void)webView:(WKWebView *)webView didFinishNavigation:(WKNavigation *)navigation {
    ...
    // Test JS engine
    [self.webView evaluateJavaScript:@"1+1" completionHandler:^(id r, NSError *err) {
        if (err) [self appLog:@"❌ JS engine test FALLITO: %@", err.localizedDescription];
        else [self appLog:@"✓ JS engine OK: 1+1=%@", r];
    }];
    [self.webView evaluateJavaScript:@"typeof maplibregl" completionHandler:^(id r, NSError *err) {
        if (err) [self appLog:@"❌ maplibregl test ERR: %@", err.localizedDescription];
        else [self appLog:@"📊 typeof maplibregl = %@", r];
    }];
    [self.webView evaluateJavaScript:@"(map ? 'definito' : 'null')" completionHandler:^(id r, NSError *err) {
        if (err) [self appLog:@"❌ map test ERR: %@", err.localizedDescription];
        else [self appLog:@"📊 map = %@", r];
    }];
