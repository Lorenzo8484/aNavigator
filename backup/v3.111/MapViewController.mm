...
- (void)applyCameraSettingsWithCoord:(CLLocationCoordinate2D)centerCoord {
    if (!_lastLocation && (centerCoord.latitude == 44.49 && centerCoord.longitude == 11.34)) {
        ...
        NSString *js = [NSString stringWithFormat:
            ...
            centerCoord.latitude, centerCoord.longitude, zoom,
            0.0,
            self.cameraPitch];
        [self.webView evaluateJavaScript:js completionHandler:nil];
        return;
    ...
    // Offset solo FUORI navigazione (durante navigazione: camera centrata sul GPS per freccia stabile)
    ...
    [self appLog:@"   target: %.6f,%.6f (nav=%d offset=%d)", target.latitude, target.longitude, self.isNavigating, !self.isNavigating];
    ...
    if (self.isNavigating) {
        ...
        [self appLog:@"   speed=%.1f km/h, alt=%.0f->%.0f", speedKmh, self.cameraAltitude, effectiveAltitude];
    }
...
- (void)setSimulatedLocation:(double)lat lon:(double)lon course:(double)course {
    ...
    // Camera segue OGNI tick (60fps) — niente scatti
    [self applyCameraSettings];
...
- (void)smoothTick {
    ...
    if (_isInterpolating) {
        ...
        // Camera segue con posizione INTERPOLATA (non _lastLocation fermo!)
        CLLocationCoordinate2D interpCoord = CLLocationCoordinate2DMake(lat, lon);
        [self appLog:@"smoothTick: t=%.3f interpCoord=%.6f,%.6f (calling applyCameraSettingsWithCoord)", t, lat, lon];
        [self applyCameraSettingsWithCoord:interpCoord];
    } else {
        // Non in interpolazione: toggle log ogni 100 chiamate
        static int _logCounter = 0;
        _logCounter++;
        if (_logCounter % 100 == 0)
            [self appLog:@"smoothTick: NO interp, calling applyCameraSettings (counter=%d)", _logCounter];
        [self applyCameraSettings];
    }
}
