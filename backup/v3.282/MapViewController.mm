...
- (void)viewWillTransitionToSize:(CGSize)size withTransitionCoordinator:(id<UIViewControllerTransitionCoordinator>)coordinator {
    ...
    [coordinator animateAlongsideTransition:nil completion:^(id<UIViewControllerTransitionCoordinatorContext> context) {
        // 1. Ricrea freccia (WKWebView perde l'icona canvas)
        [self.webView evaluateJavaScript:@"reinitArrowIcon();" completionHandler:nil];
        
        // 2. Recupera posizione corrente e forza ri-centro camera esplicito
        double lat, lon, course;
        double zoom, pitch;
        NSUserDefaults *ud = [NSUserDefaults standardUserDefaults];
        double alt = [ud objectForKey:@"autista_cam_alt"] ? [ud floatForKey:@"autista_cam_alt"] : 500;
        pitch = [ud objectForKey:@"autista_cam_pit"] ? [ud floatForKey:@"autista_cam_pit"] : 60;
        zoom = MAX(10.0, MIN(21.5, 18.0 - log2(MAX(alt, 10.0) / 100.0)));
        
        if (self->_isBusSimulating) {
            lat = self->_busSimLat;
            lon = self->_busSimLon;
            course = self->_busSimCourse;
            [self pushBusPositionToJS];
        } else if (self->_isSimulating || self.isNavigating) {
            if (!self->_lastLocation) return;
            lat = self->_lastLocation.coordinate.latitude;
            lon = self->_lastLocation.coordinate.longitude;
        ...
        NSString *js = [NSString stringWithFormat:
            @"recenterCamera(%f, %f, %f, %f, %f, %f, %f);",
            lat, lon, course, zoom, pitch, off, voff];
