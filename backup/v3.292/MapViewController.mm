...
- (void)setupSearchOverlay {
    ...
    // ⏰ Time picker per bus mode
    _searchHour = 0;
    _searchMin = 0;
    _timePicker = [[UIPickerView alloc] initWithFrame:CGRectMake(w/2 - 100, 50, 200, 100)];
    _timePicker.delegate = self;
    _timePicker.dataSource = self;
    _timePicker.alpha = 0.5;
    _timePicker.transform = CGAffineTransformMakeScale(0.7, 0.7);
    _timePicker.hidden = YES; // visibile solo in bus mode
...
- (void)openSearch {
    ...
    // ⏰ Time picker — solo in bus mode
    _timePicker.hidden = !isBus;
    _timePickerLabel.hidden = !isBus;
    if (isBus) {
        CGFloat pickerY = safeTop + 8 + 44 + 4;
        _timePicker.frame = CGRectMake(w/2 - 90, pickerY, 180, 80);
        _timePickerLabel.frame = CGRectMake(w/2 - 100, pickerY + 60, 200, 20);
    }
    ...
    [UIView animateWithDuration:0.25 animations:^{
        ...
        if ([SettingsStore shared].transportMode == TransportModeBus) {
            tableY += 70; // spazio per time picker
        }
