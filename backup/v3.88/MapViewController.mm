...
- (void)setSimulatedLocation:(double)lat lon:(double)lon course:(double)course {
    ...
    [self appLog:@"📍 simTick setLoc: %.6f,%.6f course=%.0f idx=%ld", lat, lon, course, (long)_simIndex];
    // Camera segue SOLO se simulazione attiva e non troppo spesso (riduce jitter)
    if (_isSimulating) {
        [self applyCameraSettings];
    }
    // Aggiorna JS — 60fps
    ...
    // updateArrowRotation usa _cameraHeading che è stato aggiornato da applyCameraSettings
