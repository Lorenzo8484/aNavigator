...
- (void)rebuildSliders {
    ...
    float mapBright = [udB objectForKey:@"map_brightness"] ? [udB floatForKey:@"map_brightness"] : 1.0;
    
    // ---- LUMINOSITA MAPPA ----
    UILabel *mbh = [[UILabel alloc] init]; mbh.text = LOC(@"Luminosita mappa");
    ...
    [NSLayoutConstraint activateConstraints:@[
        [mbh.topAnchor constraintEqualToAnchor:self.slidersView.topAnchor constant:290],
        [mbh.leadingAnchor constraintEqualToAnchor:self.slidersView.leadingAnchor constant:4],
    ]];
    
    [self mkSX:13 top:310 t:LOC(@"Luminosita") mn:0.0 mx:1.0 val:mapBright fn:^(UISlider*sl){
        NSString *js = [NSString stringWithFormat:@"setMapBrightness(%f)", sl.value];
        [self.mapVC.webView evaluateJavaScript:js completionHandler:nil];
