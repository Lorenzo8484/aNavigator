...
- (void)applyAllSettings {
    ...
    // === MAPPA ===
    // Always load light tiles, brightness controlled by CSS filter
    [self.webView evaluateJavaScript:@"setMapType(0)" completionHandler:nil];
    
    // Mappa luminosità (brightness slider)
    NSUserDefaults *ud = [NSUserDefaults standardUserDefaults];
    float mapBright = [ud objectForKey:@"map_brightness"] ? [ud floatForKey:@"map_brightness"] : 1.0;
    [self.webView evaluateJavaScript:[NSString stringWithFormat:@"setMapBrightness(%f)", mapBright] completionHandler:nil];
    ...
    // === VISUALE ===
    self.view.backgroundColor = [UIColor colorWithWhite:0.15 alpha:1.0];  // Always light
