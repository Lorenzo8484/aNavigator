...
- (void)applyCameraSettings {
    // Durante navigazione/simulazione: aggiorna SOLO zoom e pitch in real-time
    // (centro e bearing gestiti dal rAF engine a 60fps)
    if (self.isNavigating || _isSimulating) {
        if (!_lastLocation) return;
        // Legge SEMPRE da SettingsStore/NSUserDefaults per riflettere slider in tempo reale
        NSUserDefaults *ud = [NSUserDefaults standardUserDefaults];
