...
- (void)rebuildSliders {
    ...
    [self mkSX:3 top:210 t:LOC(@"Tilt visuale")    mn:-180 mx:180   val:hedV fn:^(UISlider*sl){
        self.mapVC.cameraHeadingOffset=sl.value;
        self.headV.text=[NSString stringWithFormat:@"%.0f°",sl.value];
        // Solo rotazione, non sposta centro
        double bearing = fmod(sl.value + 360, 360);
        [self.mapVC.webView evaluateJavaScript:[NSString stringWithFormat:@"map.easeTo({bearing:%f, duration:200});", bearing] completionHandler:^(id r, NSError *err){
            if(err) NSLog(@"Tilt JS err: %@", err.localizedDescription);
        }];
    } minL:@"-180°" midL:@"0°" maxL:@"+180°" s:&a4 l:&v4 save:@"cam_hed" cam:NO];
