...
- (void)applyCameraSettingsWithCoord:(CLLocationCoordinate2D)centerCoord {
    static double _lastCamMs = 0;
    double nowMs = [[NSDate date] timeIntervalSince1970] * 1000;
    double deltaMs = _lastCamMs > 0 ? (nowMs - _lastCamMs) : 0;
    _lastCamMs = nowMs;
...
- (void)setSimulatedLocation:(double)lat lon:(double)lon course:(double)course {
    ...
    // Rendering gestito da smoothTick (60fps con jumpTo)
}
...
- (void)smoothTick {
    ...
    // TUTTO in UNA evalJS: Marker + rotazione + camera jumpTo — sincrono a 60fps = fluido come video
    if (_isSimulating) {
        ...
        // UNA sola evalJS: Marker + rotazione + jumpTo — 60fps sincrono
        NSString *js = [NSString stringWithFormat:
            @"smoothNavFrame(%f,%f,%f,%f,%f,%f,%f,%f);",
            lat, lon, camLat, camLon, course, zoom, self.cameraPitch,
            [SettingsStore shared].arrowRotation];
