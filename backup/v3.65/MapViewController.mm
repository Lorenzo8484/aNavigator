...
- (void)simulationButtonTapped {
    ...
    // Ottieni coordinate route da JS — usa navCoords direttamente
    NSString *getCoordsJs = @"JSON.stringify(navCoords.map(function(c){ return {lat:c[1], lon:c[0]}; }))";
    [self.webView evaluateJavaScript:getCoordsJs completionHandler:^(id result, NSError *error) {
        MapViewController *strongSelf = self;
        if (!strongSelf) return;
        if (error || ![result isKindOfClass:[NSString class]]) {
            [strongSelf appLog:@"❌ simulGetCoords: %@", error.localizedDescription ?: @"result not string"];
            return;
        }
        NSData *data = [(NSString *)result dataUsingEncoding:NSUTF8StringEncoding];
        if (!data) { [strongSelf appLog:@"❌ simulGetCoords: encoding fallito"]; return; }
        ...
        if (![coords isKindOfClass:[NSArray class]] || coords.count < 2) {
            [strongSelf appLog:@"❌ simulGetCoords: %lu coord insufficienti", (unsigned long)[coords count]];
            return;
        }
        
        dispatch_async(dispatch_get_main_queue(), ^{
            [strongSelf appLog:@"✅ Simulazione: %lu coordinate", (unsigned long)[coords count]];
            
            // Avvia navigazione (route già presente dal tap)
            ...
            strongSelf->_simStepPerTick = 8.33 * 0.2; // 30 km/h ≈ 8.33 m/s * 0.2s = 1.66m per tick
            
            // Imposta posizione iniziale sul primo punto
            NSDictionary *first = coords[0];
            [strongSelf setSimulatedLocation:[first[@"lat"] doubleValue] lon:[first[@"lon"] doubleValue] course:0];
