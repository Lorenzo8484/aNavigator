...
- (void)applyCameraSettings {
    ...
    // Offset: durante simulazione DIETRO la freccia, altrimenti laterale
    CLLocationCoordinate2D centerCoord = _lastLocation.coordinate;
    double offsetLat, offsetLon;
    if (_isSimulating) {
        // DIETRO la freccia: opposto alla direzione di marcia
        offsetLat = -self.cameraOffset * cos(headingRad) / 111320.0;
        offsetLon = -self.cameraOffset * sin(headingRad) / (111320.0 * cos(centerCoord.latitude * M_PI / 180.0));
    } else {
        // Laterale (comportamento preesistente)
        offsetLat = self.cameraOffset * cos(headingRad + M_PI_2) / 111320.0;
        offsetLon = self.cameraOffset * sin(headingRad + M_PI_2) / (111320.0 * cos(centerCoord.latitude * M_PI / 180.0));
    }
...
- (void)setSimulatedLocation:(double)lat lon:(double)lon course:(double)course {
    ...
    // Camera segue la freccia a ogni tick simulazione (fluido)
    if (_isSimulating) {
        [self applyCameraSettings];
    }
}
...
- (void)simTick {
    if (!_isSimulating || !_simCoords || _simIndex >= (NSInteger)[_simCoords count] - 1) {
        if (_isSimulating) {
            [self appLog:@"✅ simTick completata (idx=%ld >= %ld)", (long)_simIndex, (long)[_simCoords count] - 1];
            _isSimulating = NO;
            [_simTimer invalidate]; _simTimer = nil;
            _simCoords = nil;
        }
    ...
    double remain = _simStepPerTick; // 1.666m per tick
    ...
    [self appLog:@"⚡ simTick: remain=%.3f idx=%ld/%lu cur=%.6f,%.6f",
     remain, (long)_simIndex, (unsigned long)[_simCoords count], curLat, curLon];
    ...
    while (remain > 0 && _simIndex < (NSInteger)[_simCoords count] - 1) {
        ...
        [self appLog:@"  ▶ next[%ld] dist=%.3fm, remain=%.3f", (long)(_simIndex+1), distToNext, remain];
        ...
        if (distToNext <= remain + 0.001) {
            ...
            curLat = nextLat;
            curLon = nextLon;
            [self appLog:@"  ✅ RAGGIUNTO punto %ld! remain=%.3f", (long)_simIndex, remain];
        } else {
            ...
            remain = 0;
            [self appLog:@"  🔄 Interpolato: cur=%.6f,%.6f (frac=%.6f)", curLat, curLon, frac];
        }
    }
