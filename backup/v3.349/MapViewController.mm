...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component {
    if (pickerView == _dayPicker) return 31 * 5;   // 5 cicli
    if (pickerView == _monthPicker) return 12 * 5;  // 5 cicli
    if (pickerView == _hourPicker) return 24 * 5;   // 5 cicli
    if (pickerView == _minPicker) return 60 * 5;    // 5 cicli
    return 0;
}
...
- (void)openSearch {
    ...
    if (isBus) {
        ...
        [_dayPicker selectRow:(31 * 2 + (_selectedDay - 1)) inComponent:0 animated:NO];
        [_monthPicker selectRow:(12 * 2 + (_selectedMonth - 1)) inComponent:0 animated:NO];
        [_hourPicker selectRow:(24 * 2 + _searchHour) inComponent:0 animated:NO];
        [_minPicker selectRow:(60 * 2 + _searchMin) inComponent:0 animated:NO];
