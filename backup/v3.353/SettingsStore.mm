...
- (NSArray<NSDictionary *> *)recentDestinationsForMode:(TransportMode)mode {
    ...
    // Migrazione: se in bus mode e ci sono vecchi bus recent in recentDests, spostali
    if (mode == TransportModeBus) {
        ...
        if (oldArr) {
            ...
            for (NSDictionary *old in [oldArr copy]) {
                if ([old[@"isBus"] boolValue]) {
                    // Migra a recentDestsBus
                    [arr addObject:old];
...
- (void)removeRecentDestination:(NSDictionary *)dest forMode:(TransportMode)mode {
    ...
    [[NSUserDefaults standardUserDefaults] setObject:arr forKey:key];
    // Se rimosso da questa lista, rimuovi ANCHE dall'altra (per migrazione vecchi dati)
    if (removed) {
        NSString *otherKey = [self _recentKeyForMode:(mode == TransportModeBus) ? TransportModeAuto : TransportModeBus];
        NSMutableArray *otherArr = [[[NSUserDefaults standardUserDefaults] arrayForKey:otherKey] mutableCopy] ?: [NSMutableArray array];
        if (lineNum) {
            for (NSDictionary *existing in [otherArr copy]) {
                if ([existing[@"lineNumber"] isEqualToString:lineNum]
                    && [existing[@"direction"] integerValue] == [dest[@"direction"] integerValue]) {
                    [otherArr removeObject:existing];
                    break;
                }
            }
        } else if (name) {
            [otherArr removeObject:@{@"name": name}];
        }
        [[NSUserDefaults standardUserDefaults] setObject:otherArr forKey:otherKey];
    }
}
