...
- (void)pushCameraSettingsToJS {
    // Camera handled by ObjC applyCameraSettings
}
...
- (void)simulationButtonTapped {
    ...
    if (isBus) {
        // === SIMULAZIONE BUS ===
        // NON chiamiamo startBusNavigation() (parte da userPos + avvia pushTimer che compete con simTick)
        // Invece: route è già visibile da selectBusLine(), prendiamo direttamente _busOsrmCoords
        _isSimulating = YES;
        _simCoords = nil;
        _simIndex = 0;
        _simStepPerTick = 0.1389; // ~30 km/h
        // Ottieni coordinate OSRM stops-only e setta navCoords in JS
        // Camera settings gestite da ObjC applyCameraSettings
    ...
    if (!_simCoords || _simCoords.count < 2) {
        ...
        [self.webView evaluateJavaScript:calcJs completionHandler:^(id r, NSError *e) {
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (_simCoords && _simCoords.count >= 2) {
                    _isSimulating = YES; // SUBITO prima di startNavigationTo per evitare conflitto buffer
...
- (void)setSimulatedLocation:(double)lat lon:(double)lon course:(double)course {
    ...
    // Camera segue OGNI 12 TICK (~200ms = 5fps) — come v3.193
...
- (void)startNavigationTo:(NSDictionary *)destinationDict {
    ...
    // Avvia NavEngine JS (rAF loop) per freccia + camera fluida SOLO per navigazione reale
    // Aggiorna posizione freccia SUBITO con la posizione corrente
    if (!_isSimulating) {
        double initLat = _lastLocation ? _lastLocation.coordinate.latitude : 44.49;
        double initLon = _lastLocation ? _lastLocation.coordinate.longitude : 11.34;
        [self pushPositionToJSWithLat:initLat lon:initLon course:0];
        [self appLog:@"   ✅ Posizione iniziale inviata a updatePosition"];
    }
...
- (void)endNavigation {
    ...
    [self.webView evaluateJavaScript:@"endNavigation(); clearRoute(); clearDestination(); clearBusRouteLayers();" completionHandler:^(id res, NSError *err) {
