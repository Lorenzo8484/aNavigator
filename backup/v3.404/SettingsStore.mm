...
- (instancetype)init {
    if (self = [super init]) {
        ...
        _cacheSize = 2.3;
        _privacyData = @""; _openSourceLicenses = @""; _appVersion = @"1.4";
        _attributions = @""; // caricato sotto
        
        _autoBrightness = YES; _darkTheme = NO; _textSize = @"Normale";
...
- (void)save {
    ...
    [ud setFloat:_arrowRotation forKey:@"set_arrR"]; [ud setFloat:_mapBrightness forKey:@"set_mapBri"];
    [ud synchronize];
}
...
#pragma mark - Destinazioni recenti

- (NSString *)_recentKeyForMode:(TransportMode)mode {
    return (mode == TransportModeBus) ? @"recentDestsBus" : @"recentDests";
}

- (NSArray<NSDictionary *> *)recentDestinations {
    NSArray *arr = [[NSUserDefaults standardUserDefaults] arrayForKey:@"recentDests"];
    return arr ?: @[];
}

- (NSArray<NSDictionary *> *)recentDestinationsForMode:(TransportMode)mode {
    NSString *key = [self _recentKeyForMode:mode];
    NSMutableArray *arr = [[[NSUserDefaults standardUserDefaults] arrayForKey:key] mutableCopy];
    if (!arr) arr = [NSMutableArray array];
    
    // Migrazione: se in bus mode sposta vecchi bus recent da recentDests a recentDestsBus
    // (i vecchi bus recent non hanno isBus=YES ma hanno lineNumber)
    if (mode == TransportModeBus) {
        NSMutableArray *oldArr = [[[NSUserDefaults standardUserDefaults] arrayForKey:@"recentDests"] mutableCopy];
        if (oldArr && oldArr.count > 0) {
            BOOL migrated = NO;
            for (NSDictionary *old in [oldArr copy]) {
                if ([old[@"lineNumber"] length] > 0) {
                    // Aggiungi campi mancanti per compatibilità
                    NSMutableDictionary *migratedEntry = [old mutableCopy];
                    if (!migratedEntry[@"isBus"]) migratedEntry[@"isBus"] = @YES;
                    if (!migratedEntry[@"departure"]) migratedEntry[@"departure"] = @"";
                    if (!migratedEntry[@"dirLabel"]) migratedEntry[@"dirLabel"] = @"";
                    if (!migratedEntry[@"stopCount"]) migratedEntry[@"stopCount"] = @(0);
                    if (!migratedEntry[@"_patIdx"]) migratedEntry[@"_patIdx"] = @(-1);
                    if (!migratedEntry[@"_tripIdx"]) migratedEntry[@"_tripIdx"] = @(-1);
                    [arr addObject:migratedEntry];
                    [oldArr removeObject:old];
                    migrated = YES;
                }
            }
            if (migrated) {
                [[NSUserDefaults standardUserDefaults] setObject:arr forKey:@"recentDestsBus"];
                [[NSUserDefaults standardUserDefaults] setObject:oldArr forKey:@"recentDests"];
            }
        }
    }
...
- (void)addRecentDestination:(NSDictionary *)dest {
    [self addRecentDestination:dest forMode:TransportModeAuto];
}

- (void)addRecentDestination:(NSDictionary *)dest forMode:(TransportMode)mode {
    NSString *key = [self _recentKeyForMode:mode];
    NSMutableArray *arr = [[[NSUserDefaults standardUserDefaults] arrayForKey:key] mutableCopy] ?: [NSMutableArray array];
    // Deduplica per bus (lineNumber+direction) o auto (name)
...
- (void)clearRecentDestinations {
    ...
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:@"recentDestsBus"];
}

- (void)removeRecentDestination:(NSDictionary *)dest {
    [self removeRecentDestination:dest forMode:(dest[@"isBus"] ? TransportModeBus : TransportModeAuto)];
}

- (void)removeRecentDestination:(NSDictionary *)dest forMode:(TransportMode)mode {
    // Rimuovi da ENTRAMBE le liste (recentDests e recentDestsBus) incondizionatamente
    for (NSString *key in @[@"recentDests", @"recentDestsBus"]) {
        NSMutableArray *arr = [[[NSUserDefaults standardUserDefaults] arrayForKey:key] mutableCopy];
        if (!arr) continue;
        NSString *name = dest[@"name"];
        NSString *lineNum = dest[@"lineNumber"];
