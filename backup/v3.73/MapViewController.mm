...
- (void)fireSimulation {
    ...
    [self appLog:@"✅ Simulazione avviata: %lu coord, %.3f m/tick, 0.2s", (unsigned long)[_simCoords count], _simStepPerTick];
}
...
- (void)setSimulatedLocation:(double)lat lon:(double)lon course:(double)course {
    ...
    // Aggiorna JS
    [self appLog:@"📍 setSimLoc: %.6f,%.6f course=%.0f idx=%ld", lat, lon, course, (long)_simIndex];
...
- (void)simTick {
    if (!_isSimulating || !_simCoords || _simIndex >= (NSInteger)[_simCoords count] - 1) {
        if (_isSimulating) {
            [self appLog:@"✅ simTick: simulazione completata (idx=%ld >= %ld)", (long)_simIndex, (long)[_simCoords count] - 1];
            _isSimulating = NO;
            [_simTimer invalidate]; _simTimer = nil;
        }
        return;
    ...
    // Log pre-movimento
    [self appLog:@"⚡ simTick: remain=%.3f idx=%ld/%lu", remain, (long)_simIndex, (unsigned long)[_simCoords count]];
    ...
    while (remain > 0 && _simIndex < (NSInteger)[_simCoords count] - 1) {
        ...
        [self appLog:@"  ▶ p1[lat]=%.6f p1[lon]=%.6f p2[lat]=%.6f p2[lon]=%.6f", lat1, lon1, lat2, lon2];
        ...
        [self appLog:@"  ▶ segDist=%.6f a=%.10f c=%.10f remain=%.3f", segmentDist, a, c, remain];
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message {
    if ([message.name isEqualToString:@"speak"]) {
        ...
        if ([coords isKindOfClass:[NSArray class]] && coords.count >= 2) {
            dispatch_async(dispatch_get_main_queue(), ^{
                [self appLog:@"✅ simCoords: %lu coordinate salvate, stepPerTick=%.3f", (unsigned long)[coords count], 1.666];
                _simCoords = coords;
                _simIndex = 0;
                _simStepPerTick = 1.666;
            });
        }
