...
- (void)startNavigationTo:(NSDictionary *)destinationDict {
    // La route è già stata calcolata quando l'utente ha tappato il risultato.
    // Non serve ricalcolare — OSRM rate limita e blocca la navigazione.
    // Vai SUBITO in navigazione con la route già presente.
    ...
    // AVVICINA SUBITO la camera sulla posizione GPS con le impostazioni 3D dell'utente
    if (_lastLocation) {
        double alt = self.cameraAltitude;
        double pitch = self.cameraPitch;
        double heading = _lastLocation.course >= 0 ? _lastLocation.course + self.cameraHeadingOffset : self.cameraHeadingOffset;
    ...
    // Avvia navigazione JS (route già presente dal tap)
    ...
    // Ricalcola route in background (silenzioso se fallisce)
    double destLat = [destinationDict[@"lat"] doubleValue];
    double destLon = [destinationDict[@"lon"] doubleValue];
    NSString *destName = destinationDict[@"name"] ?: @"Destinazione";
    double fromLat = _lastLocation ? _lastLocation.coordinate.latitude : 44.49;
    ...
    [self.webView evaluateJavaScript:js completionHandler:^(id result, NSError *error) {
        if (error) {
            [self appLog:@"⚠️ Ricalcolo route in background fallito: %@", error.localizedDescription];
        } else if (!result) {
            [self appLog:@"⚠️ Ricalcolo route in background vuoto"];
        } else {
            [self appLog:@"✅ Route ricalcolata in background con posizione aggiornata"];
        }
    }];
}
