...
- (void)busSimButtonTapped {
    ...
    // Prende bus stops correnti
    if (!_busStopAnnotations || _busStopAnnotations.count < 2) {
        [self appLog:@"❌ Sim Bus: fermate insufficienti (count=%lu)", (unsigned long)[_busStopAnnotations count]];
        return;
    }
    
    // Costruisce _busSimCoords dalle fermate
    _busSimCoords = [_busStopAnnotations copy];
    _busSimStepPerTick = 10.0; // 10 m/tick a 60fps = ~36 km/h bus
    
    [self appLog:@"✅ Sim Bus: %lu fermate, %.1f m/tick", (unsigned long)[_busSimCoords count], _busSimStepPerTick];
    
    // Imposta navigazione bus
