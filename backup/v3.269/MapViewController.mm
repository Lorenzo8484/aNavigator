...
- (void)busSimButtonTapped {
    ...
    [self.webView evaluateJavaScript:@"JSON.stringify(_busNavData && _busNavData.trip && _busNavData.trip.stops ? _busNavData.trip.stops.map(function(s){return{lat:s.lat,lon:s.lon,name:s.name}}) : [])" completionHandler:^(id res, NSError *err) {
        ...
        NSArray *stops = [NSJSONSerialization JSONObjectWithData:jsonData options:0 error:nil];
        if (![stops isKindOfClass:[NSArray class]] || stops.count < 2) {
            [self appLog:@"❌ Sim Bus: fermate insufficienti dal JS (count=%lu)", (unsigned long)[stops count]];
            return;
        }
        
        [self appLog:@"✅ Sim Bus: %lu fermate ottenute dal JS", (unsigned long)[stops count]];
