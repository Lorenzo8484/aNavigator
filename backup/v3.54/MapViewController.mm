...
- (void)startNavigationTo:(NSDictionary *)destinationDict {
    ...
    [self.webView evaluateJavaScript:js completionHandler:^(id result, NSError *error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (error) {
                [self appLog:@"⚠️ Route ricalcolo fallito: %@ — parto con route già presente", error.localizedDescription];
            } else if (!result) {
                [self appLog:@"⚠️ Route ricalcolo vuoto — parto con route già presente"];
            }
